...
func (window *Window) ProcessPointerPosition() {
	if window.hasActiveOverlay {
		...
		if window.selectedWidget != nil &&
			window.selectedWidget.GetBaseWidget().widgetType == frameWidget {
...
func (window *Window) selectWidget(widget Widget) {
	if widget.GetBaseWidget().IsPointInside(window.cursorX, window.cursorY) {
...
func (window *Window) fireClickEvents(key MouseKey) {
